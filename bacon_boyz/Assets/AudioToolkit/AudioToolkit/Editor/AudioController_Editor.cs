...
public class AudioController_Editor : EditorEx
{
    ...
    public override void OnInspectorGUI()
    {
        ...
        if ( globalFoldout = EditorGUILayout.Foldout( globalFoldout, "Global Audio Settings", foldoutStyle ) )
        {
            ...
            EditBool( ref AC.UsePooledAudioObjects, "Use Pooled AudioObjects", "Pooling increases performance when playing many audio files. Strongly recommended particuliarly on mobile platforms." );
        ...
        { 
            EditBool( ref AC.specifyCrossFadeInAndOutSeperately, "Separate crossfade in/out", "Allows to specify a seprate fade-in and out value ofr all music");
            ...
            if ( curCat != null )
            {
                ...
                if ( itemFoldout = EditorGUILayout.Foldout( itemFoldout, "Audio Item Settings", foldoutStyle ) )
                {
                    ...
                    if ( curItem != null )
                    {
                        ...
                        if ( curItem.Loop == AudioItem.LoopMode.LoopSequence || curItem.Loop == AudioItem.LoopMode.PlaySequenceAndLoopLast )
                        {
                            EditInt( ref curItem.loopSequenceCount, "   Stop after subitems", "", "Playing will stop after this number of different subitems were played. Specify zero to play endlessly in LoopSequence mode or play all sub-items in PlaySequenceAndLoopLast mode" );
                            ...
                            EditFloat( ref curItem.loopSequenceRandomDelay, "   Random Delay", "sec", "A random delay between 0 and this value will be added between two subsequent subitmes. Can be combined with the 'Overlap' value." );
                        }
